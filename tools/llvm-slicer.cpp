...
llvm::cl::opt<std::string> slicing_criterion(
    "c", llvm::cl::Required,
    llvm::cl::desc(
    ...
    llvm::cl::value_desc("func"), llvm::cl::init(""),
    llvm::cl::cat(SlicingOpts));

llvm::cl::opt<uint64_t> pta_field_sensitivie(
    "pta-field-sensitive",
    llvm::cl::desc(
...
llvm::cl::opt<PtaType> pta(
    ...
    llvm::cl::values(
        ...
        clEnumVal(fs, "Flow-sensitive PTA"), nullptr),
    llvm::cl::init(fi), llvm::cl::cat(SlicingOpts));

llvm::cl::opt<CD_ALG> CdAlgorithm(
    "cd-alg", llvm::cl::desc("Choose control dependencies algorithm to use:"),
    llvm::cl::values(clEnumValN(CLASSIC, "classic",
                                "Ferrante's algorithm (default)"),
                     clEnumValN(CONTROL_EXPRESSION, "ce",
                                "Control expression based (experimental)"),
                     nullptr),
...
class CommentDBG : public llvm::AssemblyAnnotationWriter {
  ...
  void printPointer(const analysis::pta::Pointer &ptr,
    ...
    if (!ptr.isUnknown()) {
      if (ptr.isNull())
        ...
        const llvm::Value *val = ptr.target->getUserData<llvm::Value>();
        printValue(val, os);
      }
  ...
  void emitNodeAnnotations(LLVMNode *node, llvm::formatted_raw_ostream &os) {
    if (opts & ANNOTATE_RD) {
      ...
      if (params && !RD) {
        for (auto it : *params) {
          os << "  ; PARAMS: in " << it.second.in << ", out " << it.second.out
             << "\n";
        ...
        for (auto it = params->global_begin(), et = params->global_end();
             it != et; ++it) {
          os << "  ; PARAM GL: in " << it->second.in << ", out "
             << it->second.out << "\n";
...
public:
  CommentDBG(uint32_t o = ANNOTATE_DD, LLVMReachingDefinitions *rd = nullptr)
      : RD(rd), opts(o) {}
...
protected:
  ...
  bool sliceGraph(LLVMDependenceGraph &d) {
    ...
    // 2016.11.3 jiangg merge upstream/master
    std::set<LLVMNode *> callsites;

    // TODO: Support multiple criterions.
    // for (const std::string &SC : slicing_criterion) {
    const std::string &SC = slicing_criterion.getValue();
...
static bool array_match(const std::string &name,
...
static int save_module(llvm::Module *M, bool should_verify_module = true) {
